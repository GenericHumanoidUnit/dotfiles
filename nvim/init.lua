...
do
  ...
  vim.keymap.set( 'n', '<C-b>', ':bp<CR>', { noremap = true, silent = true })
...
do
  ...
  require('gitsigns').setup {
    ...
    on_attach = function(bufnr)
      ...
      vim.keymap.set('n', '<leader>bl', require('gitsigns').blame_line, { buffer = bufnr, desc = 'Git [B]lame [L]ine' })
    end,
  }
  ...
  vim.keymap.set("n", "<leader>tt", ":NvimTreeToggle<CR>", { desc= "[T]oggle Nvim[T]ree", silent = true })
end
...
do
  ...
  --yaml companion
  vim.pack.add {gh 'mosheavni/yaml-companion.nvim'}
  local yamlcomp_cfg = require("yaml-companion").setup({
    datree = {
      cache_ttl = 86400, -- Cache TTL in seconds (0 = never expire, -1 = disable)
      raw_content_base = "https://raw.githubusercontent.com/datreeio/CRDs-catalog/main/",
    },
  })
  vim.lsp.config("yamlls", yamlcomp_cfg)
  ...
  vim.keymap.set( 'n', '<leader>yc', require("yaml-companion").open_ui_select, { desc = 'Open [y]aml [c]companion schema select' })
  vim.keymap.set( 'n', '<leader>yd', function ()
    require("yaml-companion").open_datree_crd_select("lsp")
  end, { desc = 'Open [y]aml [d]companion schema select' })
...
do
  ...
  local parsers =  { 'bash', 'c', 'cpp', 'go', 'python', 'helm', 'clojure', 'tsx', 'typescript', 'terraform',
                     'yaml','java','svelte', 'javascript',
                     'diff', 'html', 'lua', 'luadoc', 'markdown', 'markdown_inline', 'query', 'vim', 'vimdoc' }
